...
app.get('/projects', function(req, res){      //Once node index.js is running. http://localhost:3000/ should return the page. hence the '/' which referres to just localhost:300 page
  pg.connect(process.env.DATABASE_URL, function(err, client, done){ //user name is postgres: and password is 12345  \conninfo on psql to get the username info. \password to change the password database is called test
      ...
      app.post('/about', function(req,res){  //Reads stuff from the locahost location /
        pg.connect(process.env.DATABASE_URL, function(err, client, done){ //Connects to the database
